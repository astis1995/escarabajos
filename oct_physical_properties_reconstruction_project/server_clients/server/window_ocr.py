...
icons_path = r"C:\Users\Oqlabscope\vinicio\icons"
...
def find_phrase(image_np, phrase):
    words = phrase.split(" ")
    words_lower = [word.lower() for word in words]
    print("words_lower", words_lower)
    
    all_words_on_screen = True
    for word in words:
        location = find_word(image_np, word)
        if not location:
            all_words_on_screen = False
            break
            
    return all_words_on_screen
...
def get_phrase_location(window_title, search_input, xmin=None, xmax=None, ymin=None, ymax=None):
    ...
    if os.path.isfile(search_input):
        ...
        result = find_phrase(sub_image, search_input)
        print(f"All words on screen: {result}")
    if result:
        x, y, w, h = result
...
def search_window(window_title, search_input, xmin=None, xmax=None, ymin=None, ymax=None):
    ...
    if os.path.isfile(search_input):
        print(f"{os.path.isfile(search_input)} is file?")
        result = find_icon(sub_image, search_input)
    else:
        result = find_phrase(sub_image, search_input)
        print(f"All words on screen: {result}")
...
def search_icon(window_title, icon_name, xmin=None, xmax=None, ymin=None, ymax=None):
    try:
       icon_path = icons_path + f"\\{icon_name}.png"
       result = search_window(window_title, icon_path, xmin, xmax, ymin, ymax)
       print("result:", result)
       return result
