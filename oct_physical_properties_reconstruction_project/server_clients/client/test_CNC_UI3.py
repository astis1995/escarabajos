...
def create_ui():
    ...
    def send_start_scan():
        ...
        request_3d_scan(scan_type=function, folder=folder, filename=filename)
    ...
    tk.Button(window, text="Start scan", command=send_start_scan).grid(row=13, column=2)
    tk.Button(window, text="Stop scan", command=send_stop_scan).grid(row=13, column=3)
