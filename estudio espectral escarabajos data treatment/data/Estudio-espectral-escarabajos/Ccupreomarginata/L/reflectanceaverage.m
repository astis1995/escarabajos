...
xlabel('\lambda (nm)');
ylabel('R (%)');
title('Average reflectance for N=6 \textit{C. cupreomarginata} specimens ','from Pacific and Central regions,LHCP', 'Interpreter', 'latex');
legend('std dev', 'Average', 'Average ± std dev');

hold off;

%%
writematrix(mean_data,"cupreo-averageL.csv")
writematrix(std_data,"cupreoL-std.csv")
