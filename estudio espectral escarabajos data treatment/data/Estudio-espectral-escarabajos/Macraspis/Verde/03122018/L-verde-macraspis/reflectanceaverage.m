...
xlabel('\lambda (nm)');
ylabel('R (%)');
title(['Average reflectance for N=11 green \textit{Macraspis} specimens,LHCP '], 'Interpreter', 'latex');
legend('std dev', 'Average', 'Average ± std dev');

hold off;

%%
writematrix(mean_data,"macraspis-green-averageL.csv")
writematrix(std_data,"macraspis-green-stdL.csv")
