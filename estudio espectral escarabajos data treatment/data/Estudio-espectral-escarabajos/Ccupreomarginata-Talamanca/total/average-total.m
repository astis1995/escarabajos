...
K=size(files);
J=K(1)/6;
lambda=importdata(files(5).name,',',171).data;

%%
for p=1:2
    xtemp=importdata(files(p).name,',',171).data;
    main(:,p)=xtemp;
    xtemp=importdata(files(p+2).name,',',171).data;
    std(:,p)=xtemp;
end
%%
...
xlabel('\lambda (nm)');
ylabel('R (%)');
title('Average reflectance for N=15 \textit{C. cupreomarginata} specimens ','from Talmanca Mountain Range, total', 'Interpreter', 'latex');
...
%%
writematrix(mean_data,"cupreoT-averagetot.csv")
writematrix(std_data,"cupreoTtot-std.csv")