...
def feature_and_label_extractor(Metric, spectra, debug = False):
    if debug:
...
class Gamma_Area_Under_Curve_First_Min_Cut(Metric):
    """This method calculates the area for the visible region (starting at 450 nm)
    ...
    def __init__(self, spectrum, config_file):
        self.spectrum = spectrum
        self.metric_value = self.set_metric_value(spectrum)
    ...
    def set_metric_value(self, spectrum, start_wavelength = None, end_wavelength = None, debug=False):
        ...
        # If no minimum found, try after second maximum
        ...
        # Check if minimum is within visible range
        if min_in_between_x <= self.visible_range_start_wavelength:
            min_in_between_x = start_wavelength
            (
                f"First minimum wavelength {min_in_between_x} is not greater than "
                f"visible range start wavelength {self.visible_range_start_wavelength} "
                f"for spectrum {spectrum.get_filename()}"
            )
        
        # Find second minimum after first minimum
    ...
    @staticmethod
    def description():
        return f"""This algorithm calculates the area for the visible region (starting at {Gamma_Area_Under_Curve_First_Min_Cut.visible_range_start_wavelength}
        and ending in the first minima between the maximum in the visible range and the maximum in the IR range.
        Then calculates the area of the IR range up to the second minimum. The ratio between these two areas is the gamma value."""
