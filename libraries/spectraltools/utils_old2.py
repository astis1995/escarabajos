...
def check_fluorometer_file(file, debug = False):
...
def check_CRAIC_file(file, debug = False):
    if Path(file).name.startswith("."):
        return False
...
def check_l1050_file(file, debug = False):
    if Path(file).name.startswith("."):
        return False
...
def check_avantes_avalight_file(file, debug = False):
    if Path(file).name.startswith("."):
        return False
    ...
    with open(file) as f:
        first_line = f.readline()
        # Check if the first line is a valid sample name (alphanumeric with spaces, no specific regex)
...
def read_l1050_file(file, debug = False):
    return get_metadata_and_dataframe_l1050(file)

def read_CRAIC_file(file, debug = False):
    return get_metadata_and_dataframe_CRAIC(file)

def read_fluorometer_file(file, debug = False):
    return get_metadata_and_dataframe_fluorometer(file)

def read_avantes_avalight_file(file, debug = False):
    return get_metadata_and_dataframe_avantes_avalight(file)

def read_spectrum_file(file, debug = False):
    if debug:
        print("file", file)
    if check_l1050_file(file, debug = debug):
        if debug:
            print("L1050 file detected")
        return read_l1050_file(file, debug = debug)
    elif check_CRAIC_file(file, debug = debug):
        if debug:
            print("CRAIC file detected")
        return read_CRAIC_file(file, debug = debug)
    elif check_fluorometer_file(file, debug = debug):
        if debug:
            print("FLUOROMETER file detected")
        return read_fluorometer_file(file, debug = debug)
    elif check_avantes_avalight_file(file, debug = debug):
        if debug:
            print("Avantes Avalight, fiber optics file detected")
        return read_avantes_avalight_file(file, debug = debug)
    else:
...
def get_metadata_and_dataframe_l1050(file_location):
    ...
    df = pd.read_csv(file_location, sep="\t", decimal=",", names=["wavelength", metadata["measuring_mode"]], skiprows=90).dropna()
    df["wavelength"], df[metadata["measuring_mode"]] = df["wavelength"].astype(float), df[metadata["measuring_mode"]].astype(float)
...
def get_metadata_and_dataframe_fluorometer(file_location):
    def get_sample_code_from_filename(row_str, file_location):
        ...
        p = re.compile(re1)
        m = p.match(filename)
        return m.group(1) if m else get_sample_code(row_str)
...
def get_metadata_and_dataframe_avantes_avalight(file_location):
    ...
    with open(file_location, encoding="latin1") as f:
        for index, row in enumerate(f):
            ...
            if index + 1 == 6:
                break
    metadata["filename"] = file_location
    metadata["measuring_mode"] = "%R"  # Reflectance, as indicated by file data
    metadata["units"] = "nm"
    ...
    df = pd.read_csv(file_location, sep=";", decimal=",", 
                     names=["wavelength", "sample", "dark", "reference", "reflectance"], 
                     skiprows=7).dropna()
    df["wavelength"] = df["wavelength"].astype(float)
    df["reflectance"] = df["reflectance"].astype(float)
    
    # Filter wavelengths < 2000 nm, consistent with L1050
    df = df[df["wavelength"] < 2000].reset_index(drop=True)
    ...
    metadata["maximum_wavelength"] = df["wavelength"].max()
    metadata["step"] = np.round(np.mean(df["wavelength"].diff()), 8)
    metadata["number_of_datapoints"] = len(df["reflectance"])
    metadata["maximum_measurement"] = df["reflectance"].max()
    metadata["minimum_measurement"] = df["reflectance"].min()
